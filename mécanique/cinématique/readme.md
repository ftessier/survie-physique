...
Mais quand Newton a formalisé tout ça au 17e siècle, personne n'avait d'outil capable de mesurer une vitesse *instantanée*. Le concept existait en mathématiques, mais pas dans les mains. La synchronisation précises des horloges commençait à peine à devenir essentielle pour la navigation. En fait, l'idée même d'un temps _absolu_ et d'un espace _absolu_, c'était un *choix* philosophique — hérité de Platon, malheureusement diront certains, et consolidé par Newton — pas une évidence. Ça avait l'air raisonnable, et ça a tenu pendant deux cents ans. Jusqu'à Einstein... mais ça, c'est une autre histoire.

Pour l'instant, le choix de Newton semble coller avec notre réalité de tous les jours. Ton cellulaire est synchronisé, un mètre a la même longueur partout, et on a des problèmes à résoudre.
...
Le symbole $\Delta$ — prononcé "Delta", la lettre grecque qui correspond à notre "D" majuscule — signifie couramment « la différence entre la valeur finale et la valeur initiale ». Quand on écrit $\Delta t$, c'est clair: c'est $t_f - t_i$, la différence entre deux instants. Quand on écrit $\Delta v$, c'est $v_f - v_i$, la différence entre deux vitesses. Parfait.
...
Et il y a un deuxième piège: la lettre $d$ peut aussi évoquer le mot *distance*, qui est un concept différent du déplacement. Le déplacement est un _vecteur,_ qui comporte une grandeur et une direction: représenté par une "flèche" de la position initiale à la position finale, peu importe le chemin). La distance est un *scalaire,* soit une simple quantité sans direction: la longueur totale du trajet parcouru. Ainsi un aller-retour au dépanneur donne un déplacement nul mais une distance bien réelle. Même lettre, deux idées opposées. Ça n'arrange rien.
