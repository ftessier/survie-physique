...
Le symbole $\Delta$ — prononcé "Delta", la lettre grecque correspondant à notre "D" majuscule — signifie souvent « la différence entre la valeur finale et la valeur initiale ». Quand on écrit $\Delta t$, c'est clair: c'est $t_f - t_i$, la différence entre deux instants. Quand on écrit $\Delta v$, c'est $v_f - v_i$, la différence entre deux vitesses. Parfait.
